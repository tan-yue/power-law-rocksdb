...
Hasher::Hasher(const int& context_size) : context_pos_(0), context_size_(context_size), hash_(0) {
  context_ = new unsigned long  long[context_size];
  pow_ = 1;
...
unsigned long long Hasher::HashLong(const unsigned long long& c) {
  hash_ = hash_ * kPower + c;
  unsigned long long old_val = context_[context_pos_];
  context_[context_pos_] = c;
  ++context_pos_;
  context_pos_ %= context_size_;
  hash_ -= pow_ * old_val;
