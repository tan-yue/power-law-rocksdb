...
#include "db/space_saving_runner.h"
#include <iostream>
#include <fstream>
#include "db/hasher.h"
...
Status WalManager::genTopkForPartitioner(uint64_t log) {
    ...
    struct LogReporter : public log::Reader::Reporter {
        ...
        void Corruption(size_t bytes, const Status& s) override {
            ...
            ROCKS_LOG_ERROR(info_log, "Log #%" PRIu64 ": dropping %d bytes; %s", lognum, static_cast<int>(bytes), s.ToString().c_str());
        }
    };
    ...
    Status status = env_->NewSequentialFile(logname, &lfile, env_->OptimizeForLogRead(env_options_));
    if (!status.ok()) {
        return status;
    }
    std::unique_ptr<SequentialFileReader> lfile_reader(new SequentialFileReader(std::move(lfile), logname));
    ...
    Slice record;
    WriteBatch batch;
	ROCKS_LOG_INFO(db_options_.info_log, "ASH : Reading log %d", log);
...
Status WalManager::DeleteFile(const std::string& fname, uint64_t number) {
  /* call the space saving algorithm here */
  //char filename[9] = "file.txt";
  //char *filename = new char[fname.length() + 1];
  //(void)space_saving_main(filename, 1, 5, 3);

  ROCKS_LOG_WARN(db_options_.info_log, "About to delete a WAL file using db_options_.info_log");
  //(void)genTopkForPartitioner(number);
...
void WalManager::PurgeObsoleteWALFiles() {
  ROCKS_LOG_INFO(db_options_.info_log, "Inside purge wal manager");
